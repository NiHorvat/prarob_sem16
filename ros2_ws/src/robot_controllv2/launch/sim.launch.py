...
def generate_launch_description():
    ...
    rviz_config = os.path.join(
        get_package_share_directory(package_name), "launch", "prarob_manipulator.rviz")

    robot_description = os.path.join(
        get_package_share_directory(package_name), "urdf", robot_name + ".urdf.xacro")
    robot_description_config = xacro.process_file(
        robot_description, mappings={"use_mock": "true"})
    ...
    return LaunchDescription([
        ...
        Node(
            package="robot_controllv2",
            executable="move_servos_node",
            name="move_servos_node",
